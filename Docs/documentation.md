...
![Karpenter Autoscaling](../static/karpenter_Autoscaling_v2.png)
...
## 2. Full Training Pipeline setup
### Build and Push pipeline containers to Amazon ECR
...
### Compile the full training pipeline
```bash
...
```
#### Create/choose experiment, fill out all the details and hit start.
![Initial run parameters](../static/initial_run.png)

* A successful run looks like so:
    ![Initial run showing logs](../static/initial_with_logs.png)


## 3. Incremental Training Pipeline Setup

### 1. Upload new interaction data to a `new_data` path in the same S3 bucket 
```bash
aws s3 cp new_data/ s3://$BUCKET/new_data/ --recursive
```
### 2. Compile the incremental preprocessing pipeline:
```bash
python3 kubeflowpipeline_2.py \
  ...
  ```
#### Create/choose experiment, fill out all the details and hit start.
![recurring run parameters](../static/recurring_run.png)

* A successful run looks like so:
    ![Incremental run showing logs](../static/incremental_with_logs.png)
...
b. Generate the item catalog from DynamoDB (required by the demo app; `catalog.json`)
```bash
python3 - <<'EOF'
import boto3, json
